...
def get_dataset(request: Request):
    ...
    # O engine expõe o dataset em duas formas: columnar_dataset (dict) e
    # columnar_gzip (bytes JSON já comprimidos). A Response precisa dos BYTES —
    # passar o dict causava AttributeError ('dict' object has no attribute 'encode')
    # e um 500 permanente neste endpoint, travando o frontend na tela de loading.
    gz = getattr(engine, "columnar_gzip", None)
    if not gz:
        # Fallback (ex: pickle antigo sem columnar_gzip): comprime na hora e memoiza.
        import gzip as _gzip
        import json as _json
        gz = _gzip.compress(
            _json.dumps(engine.columnar_dataset, ensure_ascii=False, separators=(",", ":")).encode("utf-8"),
            compresslevel=6,
        )
        engine.columnar_gzip = gz
    return Response(
        content=gz,
        ...
        if str(r.get("Id_Processo")).strip() == clean_id or str(r.get("Numero_Requerimento")).strip() == clean_id:
            ...
            if series_list and cnpj and sec_market:
                ...
                for s_idx, s_data in enumerate(series_list):
                    ...
                    mapping = None
                    if key_exact:
                        mapping = sec_market.get(key_exact)
                    if not mapping:
                        mapping = sec_market.get(key_mmyyyy)
